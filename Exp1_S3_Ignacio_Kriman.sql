...
SELECT
    ...
    SUBSTR(NUMRUT_CLI, LENGTH(NUMRUT_CLI) - 5, 3) || '.' ||
    SUBSTR(NUMRUT_CLI, LENGTH(NUMRUT_CLI) - 2, 3) || '-' ||
    DVRUT_CLI AS "RUT CLIENTE",

    NOMBRE_CLI || ' ' || APPATERNO_CLI || ' ' || APMATERNO_CLI AS "NOMBRE COMPLETO",

    TO_CHAR(RENTA_CLI, 'FM$999G999G999', 'NLS_NUMERIC_CHARACTERS = '',.''') AS "RENTA",
...
SELECT
    ...
    TO_CHAR(
        ...
        'FM$999G999G999',
        'NLS_NUMERIC_CHARACTERS = '',.''')
    AS "PROMEDIO VALOR ARRIENDO",

    ROUND(AVG(SUPERFICIE)) AS "PROMEDIO SUPERFICIE M2",

    ROUND(AVG(VALOR_ARRIENDO / SUPERFICIE)) AS "VALOR ARRIENDO M2",

    CASE
        WHEN ROUND(AVG(VALOR_ARRIENDO / SUPERFICIE)) < 5000 THEN 'Económico'
        WHEN ROUND(AVG(VALOR_ARRIENDO / SUPERFICIE)) BETWEEN 5000 AND 10000 THEN 'Medio'
        WHEN ROUND(AVG(VALOR_ARRIENDO / SUPERFICIE)) > 10000 THEN 'Alto'
        ELSE 'N/A'
    END AS "CLASIFICACION PROPIEDAD"
